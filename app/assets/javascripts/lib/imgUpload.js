;(function($,win,doc){
    ...
    $.cieldonfileupload=function(el, options){
        ...
        self.init=function(){
            ...
            if (self.imgs_type == undefined){
              self.imgs_type = self.$el.find("div:eq(0)").context.getAttribute("data-imgs_type");
            }
        ...
        self.renderImg=function(input,img,type,fname, key){
            ...
            switch (type){
                ...
                case "6":
                    ...
                    }else if(self.imgs_type == "ec_product_ec_pictures"){
                        // 微服务商品图片上传
                        fileImg.after(html);
                        var uuid = Date.now();
                        fileImg.next().append('<input class="destroy" name="ec_product[ec_pictures_attributes]['+uuid+'][_destroy]" type="hidden">');
                        fileImg.next().append('<input class="pic_key" name="ec_product[ec_pictures_attributes]['+uuid+'][pic_key]" type="hidden" value="'+key+'">');
                    }else if(self.imgs_type == "ec_shop_ec_pictures"){
                      fileImg.after(html);
                      var uuid = Date.now();
                      fileImg.next().append('<input class="destroy" name="ec_shop[ec_pictures_attributes]['+uuid+'][_destroy]" type="hidden">')
                      fileImg.next().append('<input class="pic_key" name="ec_shop[ec_pictures_attributes]['+uuid+'][pic_key]" type="hidden" value="'+key+'">')                      
                    ...
                    }else if(self.imgs_type == "site_article_site_pictures"){
                        fileImg.after(html);
                        var uuid = Date.now();
                        fileImg.next().append('<input class="destroy" name="site_article[site_pictures_attributes]['+uuid+'][_destroy]" type="hidden">');
                        fileImg.next().append('<input class="pic_key" name="site_article[site_pictures_attributes]['+uuid+'][pic_key]" type="hidden" value="'+key+'">');
