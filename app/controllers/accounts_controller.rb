...
class AccountsController < ApplicationController
  ...
  def create
    ...
    if @account.save
      ...
      #   SmsService.new.singleSend(params[:account][:mobile],"#{params[:account][:contact]} 先生/女士，感谢您注册！帐号#{params[:account][:nickname]}，密码#{params[:account][:password]}，体验时间30天。请在电脑上登录 标途科技官网 进行设置、绑定公众号，在手机上浏览效果。如遇困难，请联系4000-365-711。祝生活愉快！")  #发送短信
      # end

      session[:employee_id] = @account.employees.first.try(:id)
