class Api::MiniProgram::CategoriesController < Api::MiniProgram::BaseController
  ...
  def index
    @categories = @current_site.ec_categories.onshelf.product_category.search(params[:search]).to_a
  ...
  def category
    @categories = @current_site.ec_categories.onshelf.product_category.limit(8)
    # respond_to do |format|
    #   format.json { render json: {categories: @categories, icon: @categories.map{|c| c.icon_url}}}
    # end
    respond_to :json
  ...
  def show
    @category = @current_site.ec_categories.onshelf.find(params[:id].to_i)
  ...
  def product
    ...
    @ec_item = @current_site.ec_items.find_by_id(params[:item_id].to_i)
