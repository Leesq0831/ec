class Api::MiniProgram::WxpayController < Api::MiniProgram::BaseController
    # skip_before_filter *ADMIN_FILTERS
  skip_before_filter :set_wx_mp_user, :set_wx_user, only: [:notify]
  ...
  def notify
    ...
    if result_code == "SUCCESS"
      ...
      form_id = payment.try(:prepay_id)
      if payment.success?
        return render text: notify_result("SUCCESS")
      else
        options = {status: 1,trade_status: "TRADE_SUCCESS", buyer_id: result[:openid] , open_id: result[:openid], total_fee: result[:total_fee].to_f/100, trade_no: result[:transaction_id], order_msg: result.to_s, gmt_payment: Time.parse(result[:time_end].to_s), prepay_id: form_id }
        write_weixinv2_log "weixin v2 pay notify info option -> #{options}"
  ...
  def pay
    ...
    if params[:openid].present?
      @wx_user = WxUser.where(openid: params[:openid]).first
    else
    ...
    @wxpay = @payment.site.payment_settings.enabled.where(payment_type_id: @payment.payment_type_id).first
  ...
  def find_payment
    ...
    return render json: {errcode: 400, errmsg: "支付单不存在"} unless @payment
  ...
  def unifiedorder
    ...
    request_options = {
      ...
      # notify_url: "https://mp.biaotuyun.com/api/mini_program/wxpay/notify",
      # notify_url: "https://mp.dingyutech.com/api/mini_program/wxpay/notify",
      trade_type: "JSAPI",
      openid: @wx_user.openid,
      sub_appid: @wxpay.sub_appid,
      sub_mch_id: @wxpay.sub_mch_id,
      sub_openid: @wx_user.openid
    }
  ...
  def response_unifiedorder
    ...
    if return_code == "SUCCESS"
      ...
      return render json: {errcode: 302, errmsg: "数据异常,请尝试重试支付" }
  ...
  def set_pay_sign_params
    ...
    pay_sign_params
    return render json: {errcode: 200, errmsg: "OK", pay_sign: @pay_sign, nonceStr: @pay_sign_nonce_str, package: "prepay_id=#{@payment.prepay_id}", signType: "#{get_sign_type}", timeStamp: @pay_sign_time_stamp }
  end
