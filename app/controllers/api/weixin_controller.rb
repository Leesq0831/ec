...
class Api::WeixinController < Api::BaseController
  ...
  def event_request
    ...
    ticket         = @xml[:Ticket]
    if ticket.present?
      qrcode = @mp_user.site.qrcodes.where(ticket: ticket).first
      if qrcode
    ...
    case event
      when 'subscribe'
        ...
        @activity = EcActivity.start.first

        if @activity.present?
          if @activity.test_openids.include?(@from_user_name)
            @wx_user.user.ec_user_activities.create(ec_activity_id: @activity.id)
          else
            @wx_user.user.ec_user_activities.where(ec_activity_id: @activity.id).first_or_create
          end
          reply = "<xml>
                    ...
                    <Description><![CDATA[#{@activity.summary}]]></Description>
                    <PicUrl><![CDATA[#{@activity.pic_url}]]></PicUrl>
                    <Url><![CDATA[#{wap_cart_items_url}]]></Url>
                    </item>
                    <item>
                    </Articles>
                  </xml>"
      ...
      when 'LOCATION'
        ...
        ''
      when 'SCAN' then respond_scan_event @xml[:EventKey]
  ...
  def respond_scan_event scene_id
    vote_item = VoteItem.find_by_id(scene_id)
    return Weixin.respond_text(@from_user_name, @to_user_name, '抱歉,没有找到该选手的信息!') if vote_item.blank?

    url = "http://#{vote_item.activity.site_id}.mshequ.winwemedia.com/activity/vote/#{vote_item.activity_id}/vote_items/#{vote_item.id}?openid=#{@from_user_name}"
    items = [{title: vote_item.name, description: vote_item.content, pic_url: vote_item.default_pic_url, url: url }]
