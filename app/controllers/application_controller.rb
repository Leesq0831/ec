...
class ApplicationController < ActionController::Base
  ...
  helper_method :current_user, :services_config, :current_shop_account, :current_sub_account, :current_shop_branch, :current_employee
  helper_method :mobile_subdomain, :mobile_domain, :admin?
  ...
  def current_employee(force_reload = false)
    @current_employee ||= if session[:employee_id]
      Employee.find(session[:employee_id])
    end
    @current_employee.reload if force_reload
    @current_employee
  end
  ...
  def require_wx_mp_user
    new_release = WxRelease.last || WxRelease.create(template_id: 0, user_version: '0')
    @wx_mp_user = current_site.wx_mp_user || WxMpUser.create(account_id: current_user.id, site_id: current_site.id, nickname: current_user.nickname, template_id: new_release.template_id, user_version: new_release.user_version)
