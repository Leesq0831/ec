...
class Biz::MicroShopBranchesController < ApplicationController
  ...
  before_filter :find_shop_branch, only: [ :edit, :update, :destroy, :highchart, :pictures, :create_pic, :toggle_sub_account, :pause, :resume ]
  ...
  def index
    ...
    @search        = @shop.shop_branches.order(:sort).includes(:city, :district).used.search(params[:search])
  ...
  def pause
    @shop_branch.pending!
    redirect_to :back, notice: '暂停营业成功'
  end

  def resume
    @shop_branch.normal!
    redirect_to :back, notice: '恢复营业成功'
  end
