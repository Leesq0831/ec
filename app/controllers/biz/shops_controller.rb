class Biz::ShopsController < ApplicationController
  ...
  def save_point
    ...
    if params[:direction_type] == '1'
      if current_shop_vip_user.increase_points!(points)
        current_shop_vip_user.point_transactions.create direction_type: PointTransaction::ADJUST_IN, points: points, site_id: current_shop_account.id, shop_branch_id: current_shop_branch.id, description: params[:description]
        flash[:notice] = "增加成功"
      else
      ...
      if current_shop_vip_user.decrease_points!(points)
        current_shop_vip_user.point_transactions.create direction_type: PointTransaction::OUT, points: points, site_id: current_shop_account.id, shop_branch_id: current_shop_branch.id, description: params[:description]
        flash[:notice] = "减少成功"
      else
  ...
  def search_activities_or_coupons
    ...
    def sign_in!(sub_account_id)
      ...
      return redirect_to shops_sign_in_path(site_id: params[:site_id]), alert: '商户没有授权该门店管理权限，请先操作权限设置再登录后台。' unless signed_in_path
