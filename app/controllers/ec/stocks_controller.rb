class Ec::StocksController < Ec::BaseController
  ...
  def index
    @search = @current_user.ec_stock_items.order('created_at desc').search(params[:search])
    @stocks = @search.page(params[:page])
  end

  def new
    @stock = @current_user.ec_stocks.new
    attrs = @current_user.ec_items.onshelf.where(ec_product_id: params[:ids]).all.map{ |item| { ec_item_id: item.id, qty: 100 } }
    @stock.ec_stock_items.build()
    render layout: 'application_pop'
  end

  def create
    @stock = @current_user.ec_stocks.new(params[:ec_stock])
    if @stock.save
      flash[:notice] = '添加成功'
  ...
  private
    ...
    def find_stock
      @stock = @current_user.ec_stocks.find(params[:id])
    end
