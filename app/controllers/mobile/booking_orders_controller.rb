class Mobile::BookingOrdersController < Mobile::BaseController
  ...
  before_filter :services_config, only: [:new]
  ...
  def new
    ...
    @booking_order.price = @booking_order.booking_item.price

    @booking_order.items_amount = @booking_order.price * @booking_order.qty
    @booking_order.deliver_amount = @booking_order.booking.deliver_amount_f
    @booking_order.total_amount = @booking_order.items_amount + @booking_order.deliver_amount
    
    if @user.vip_user
      vip_user_amount = @user.vip_user.get_pay_amount( @booking_order.items_amount )
      @booking_order.discount = @booking_order.items_amount - vip_user_amount

      @booking_order.pay_amount = vip_user_amount + @booking_order.deliver_amount
    else
      @booking_order.pay_amount = @booking_order.total_amount
    end
  end
  ...
  def create
    ...
    if @booking_item.no_limit? && @booking_order.qty.to_i > @booking_item.surplus_qty
      ...
      if @booking_order.save
        if @booking_order.pay_amount == 0
          @booking_order.paid!
          redirect_to mobile_booking_booking_order_url(@site, @booking, @booking_order)
        elsif @booking_order.cashpay?
          # @booking_order.paid!
          @booking_order.send_message
          @booking_order.send_wx_message_template
