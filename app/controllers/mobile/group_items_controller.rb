class Mobile::GroupItemsController < Mobile::BaseController
  ...
  def show
    ...
    @group_order = GroupOrder.where(id: params[:group_order_id]).first
    resource = @group_order ? @group_order : @group_item

    comments_partial(resource, @user)
  end
  ...
  def redirect_vcoolife
    return redirect_to "#{Settings.wmall_host}/wx/gotogroupdetail/#{@group_item.groupable_id}-#{@group_item.id}?site_id=#{@site.id}"if @group_item.present? && @group_item.groupable_type == "Wmall::Shop"
  end
