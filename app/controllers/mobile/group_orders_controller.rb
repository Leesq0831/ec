class Mobile::GroupOrdersController < Mobile::BaseController
  ...
  def index
    ...
    @consumed_orders = @user.group_orders.consumed.order("created_at desc")
    @consumed_no_comments_orders = @user.group_orders.consumed.includes(:comments).consumed.where("not exists (select commentable_id from comments where group_orders.id=comments.commentable_id and comments.commentable_type='GroupOrder')").order("group_orders.created_at desc")
  rescue
    render :text => "参数不正确"
  end
