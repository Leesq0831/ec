...
class Mobile::ShopOrdersController < Mobile::BaseController
  ...
  before_filter :set_payment_types, only: [:confirm, :show]
  ...
  def success
    ...
    @shop_order = ShopOrder.find(params[:id])
    takeout_range = BookRule.where(rule_type: 3, shop_branch_id: @shop_order.shop_branch_id).first

    if takeout_range.metadata[:is_limit_range] == 'true'
      real_distance = @shop_order.calculate_distance(@shop_order.shop_branch, params[:shop_order][:address])
      _kilometer = takeout_range.metadata[:kilometer]
      _money = takeout_range.metadata[:over_money]
      total_amount = @shop_order.total_amount
      if (real_distance > _kilometer.to_f && total_amount.to_f > _money.to_f) || real_distance < _kilometer.to_f
        ...
        redirect_to :back, notice: "已经超出送货距离，并且未达到超出距离后的送货金额，请重新下单或者填写地址！！！！"
      end
    else
      self.get_order
    end
  ...
  def cancel
    ...
    if @shop_order.can_cancel?
      ...
      @shop_order.send_cancel_message @shop_order.user
  ...
  def add_item
    ...
    if @user.take_out_new_user? && @product.new_user_activity? && @shop_order.shop_order_items.reload.pluck(:shop_product_id).include?(@product.id)
      ...
      @item, result = @shop_order.add_item(@product)
      return render js: "alert('当前库存不足');$('.porduct-number-of-#{@product.id}').val(#{@item.try(:qty) || 0})" unless result
    end
  ...
  def get_order
    unless params[:again]
      ...
      if @shop_order.update_attributes(params[:shop_order]) && @shop_order.payment.present? && @shop_order.unfinish?
        @shop_order.payment.update_attributes(payment_type_id: @shop_order.pay_type)
      end
      ...
      #     RestClient.post("#{Settings.app_host}/v1/igetuis/igetui_app_message", {role: 'site', role_id: @shop_order.site_id, token: @shop_order.site.account.try(:token), messageable_id: @shop_order.id, messageable_type: 'ShopOrder', source: 'shop_order', message: '您有一笔新的微外卖订单, 请尽快处理'})
      #   else
      #     RestClient.post("#{Settings.app_host}/v1/igetuis/igetui_app_message", {role: 'site', role_id: @shop_order.site_id, token: @shop_order.site.account.try(:token), messageable_id: @shop_order.id, messageable_type: 'ShopOrder', source: 'shop_table_order', message: '您有一笔新的微餐饮订单, 请尽快处理'})
      #   end
      # rescue => e
      #   Rails.logger.info "#{e}"
      # end
    ...
    if !@shop_order.cashpay? && !@shop_order.finish? # && @shop_order.total_amount > 0
      ...
      module_id = @shop_order.book_dinner? ? 3 : 4

      # @site.send_system_message({site_id: @site.id, content: "#{@shop_order.created_at.strftime('%H:%M')}收到#{@shop_order.username}用户#{@shop_order.mobile}的#{@shop_order.wx_plot_category.name}#{@repair_complain.repair? ? '报修申请' : '投诉建议'}", module_id: @repair_complain.repair? ? 1 : 2 }, SystemMessageModule.where(module_id: @repair_complain.repair? ? 1 : 2).first)
      @site.send_system_message(
          {site_id: @site.id,
           content: "#{@shop_order.created_at.strftime('%H:%M')}收到#{@shop_order.username}用户#{@shop_order.mobile}的#{@shop_order.order_type_name}订单",
           module_id: module_id
          },
          SystemMessageModule.where(module_id: module_id).first
      )
    end
