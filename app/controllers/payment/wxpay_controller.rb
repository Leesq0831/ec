class Payment::WxpayController < ApplicationController
  ...
  def pay
    ...
    if params[:openid].present?
      @wx_user = @site.wx_mp_user.wx_users.where(openid: params[:openid]).first
      @wx_user = WxUser.follow(@site.wx_mp_user, wx_user_openid: params[:openid], wx_mp_user_openid: @site.wx_mp_user.try(:openid)) unless @wx_user
    else
    ...
    @wxpay = @site.payment_settings.enabled.where(payment_type_id: @payment.payment_type_id).first
