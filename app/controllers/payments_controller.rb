...
class PaymentsController < ApplicationController
    ...
    if paymentable.present?
      if paymentable.is_a?(EcOrder)
        if params['status'].present? && params['status'] == '1'
          paymentable.paid! if paymentable.paying?
        end
        redirect_to pay_wxpay_success_url(out_trade_no: payment.out_trade_no)
  ...
  def notify
    if params[:notify_data].present?
      ...
      if params['status'].present? && params['status'] == '1'
        ...
        paymentable = payment.try(:paymentable)
        if paymentable.present?
          paymentable.pay! if paymentable.is_a?(ShopOrder)
  ...
  def merchant
    ...
    if _real_merchant_url =~ URI::regexp
      ...
      redirect_to "http://wx.91heba.com"
    end
  end
