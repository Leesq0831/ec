...
module ApplicationHelper
  ...
  def quick_link_options
    ...
    #   "微酒店订单管理" => "#{Settings.hotel_host}/wehotel-all/#{current_site.id}/getOrderList"
    # }
    [
      ['请选择', ''],
      ["首页链接", "http://#{Settings.mhostname}/wap"],
      ["分类链接", "http://#{Settings.mhostname}/wap/categories"],
      ["直送餐厅链接", "http://#{Settings.mhostname}/wap/shops"],
      ["购物车链接", "http://#{Settings.mhostname}/wap/cart_items"],
      ["个人中心链接", "http://#{Settings.mhostname}/wap/users"],
      ["我的订单链接", "http://#{Settings.mhostname}/wap/orders?type=all"]
    ]
  end
