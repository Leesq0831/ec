module MobileHelper
  ...
  def mobile_index
    if @website
      mobile_root_url(subdomain: mobile_subdomain(@website.site_id), site_id: @website.site_id, aid: session[:activity_id], openid: session[:openid], anchor: 'mp.weixin.qq.com')
    else
      ''
    end
  end
  ...
  def website_activity_link(website_menu, options = {})
    ...
    @url = case website_menu.menu_type.to_i
      ...
      when 11 then
        ...
        if website_menu.website.micro_site?
          return url = "http://api.map.baidu.com/marker?location=#{website_menu.location_y},#{website_menu.location_x}&title=#{website_menu.address}&name=标途科技&content=#{website_menu.address}&output=html&src=weiba|weiweb"
        else
