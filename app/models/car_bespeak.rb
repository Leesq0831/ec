class CarBespeak < ActiveRecord::Base
  ...
  private
    ...
    def igetui
      RestClient.post("#{Settings.app_host}/v1/igetuis/igetui_app_message", {role: 'site', role_id: site_id, token: site.account.try(:token), messageable_id: self.id, messageable_type: 'CarBespeak', source: 'car', message: '您有一个汽车的新预约，请注意查看。'})
    rescue => e
      Rails.logger.info "#{e}" 
    end
