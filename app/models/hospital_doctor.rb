...
class HospitalDoctor < ActiveRecord::Base
  store :metadata, accessors: [:notice]
  ...
  def is_full
    doctor_watches.find_each do |watch|
      return false unless watch.is_full
    end
    return true
  end

  def has_future_arrange
    doctor_watches.where("doctor_watches.start_time > :time OR (doctor_watches.start_time < :time and doctor_watches.end_time > :time)", time: Time.now).where(status: 1).count > 0
  end
