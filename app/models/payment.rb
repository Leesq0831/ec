...
class Payment < ActiveRecord::Base
  ...
  scope :proxy_pay, where(payment_type_id: [20001, 20002, 20003])
  ...
  def update_pay_result options = {}
    transaction do
      ...
      if paymentable.is_a?(EcOrder)
        paymentable.paid!(options[:open_id], options[:prepay_id])
      end
    end
  end
  ...
  def get_pay_url(options={}, *args)
    case
    when self.wxpay? || self.proxy_wxpay?
      "#{PaymentSetting::WEIXIN_PAY_URL}?out_trade_no=#{out_trade_no}&site_id=#{site_id}&openid=#{open_id.presence}&showwxpaytitle=1"
    when self.tenpay?
  ...
  def default_pay_options
    ...
    if Rails.env.production?
      domain_url = 'http://wx.91heba.com'
    elsif Rails.env.staging?
      domain_url = 'http://staging.91heba.com'
    else
      domain_url = 'http://testing.91heba.com'
    end
  ...
  #   "seller_email"=>"payment@91heba.com",
  #   "sign_type"=>"MD5",
