  ...
  def save_logo
    if logo != "unchange"
      ...
      if info['ticket'].present?
        qrcode = wx_mp_user.site.qrcodes.where(ticket: info['ticket']).first
        qrcode ||= wx_mp_user.site.qrcodes.create(site_id: site_id, name: name, expire_seconds: info['expire_seconds'].presence, action_name: "QR_LIMIT_SCENE", scene_id: scene_id, ticket: info['ticket'], description: description)
