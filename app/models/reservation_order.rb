class ReservationOrder < ActiveRecord::Base
  ...
  private
    def igetui
      RestClient.post("#{Settings.app_host}/v1/igetuis/igetui_app_message", {role: 'site', role_id: site_id, token: site.account.try(:token), messageable_id: self.id, messageable_type: 'ReservationOrder', source: 'reservation', message: '您有一笔新的微预定订单，请及时处理。'})
    rescue => e
      Rails.logger.info "#{e}"
    end
