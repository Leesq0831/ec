class ShopOrderItem < ActiveRecord::Base
  ...
  def add_default_attrs
    ...
    self.qty = 1
    self.price = self.shop_product.display_price_for(self.shop_order.user)
  ...
  def cal_shop_order_amount
    ...
    if self.shop_order.take_out?
      deliver_amount = self.shop_order.book_rule.deliver_amount_f
      self.shop_order.total_amount += deliver_amount
      self.shop_order.pay_amount += deliver_amount
    end
