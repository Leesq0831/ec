class ShopProduct < ActiveRecord::Base
  store :metadata, accessors: [:activity_type, :activity_price, :is_custom_order_count, :custom_order_count]
  ...
  validates :custom_order_count, allow_blank: true, numericality: { greater_than_or_equal_to: 0, only_integer: true }
  ...
  has_many :shop_orders, through: :shop_order_items
  ...
  def new_user_activity?
    activity_type.to_s == 'new_user_activity'
  end

  def display_price_for(user = nil)
    return price unless user

    activity_price.to_f > 0 && user.take_out_new_user? ? activity_price : price
  end
