class Site < ActiveRecord::Base
  ...
  has_many :shop_branch_sub_accounts, through: :shop_branches, source: :sub_account, conditions: "shop_branches.status >= 0"
  ...
  has_many :feedbacks
  #has_many :feedbacks, as: :user_type
  ...
  def update_all_system_messages
    if system_messages.unread.update_all(is_read: true) > 0
      uri = URI.parse("#{Settings.faye_host}/faye")
  ...
  def send_system_message(options = {}, smm = nil)
    ...
    sms = SystemMessageSetting.where(site_id: id, system_message_module_id: smm.id).first_or_create
    if sms.is_open
      sms.system_messages << SystemMessage.new(site_id: id, content: options[:content], system_message_module_id: smm.id)
      sms.save!
    else
    ...
    Rails.logger.info "account send_system_message is error: #{e}"
  end
