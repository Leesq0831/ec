  ...
  def send_old_coupon_care!
    given_coupon_ids.each do |id|
      ...
      random_receivers.take(limit_count).each do |vip_user|
        ...
        content.gsub!('{优惠券}', %Q[<a href='#{Settings.m_host}/app/vips/old_coupons?init_id=#{consume.id}'>#{activity.name}</a>])
  ...
  def send_gift_care!
    given_gift_ids.each do |id|
      ...
      random_receivers.each do |vip_user|
        ...
        content.gsub!('{礼品券}', %Q[<a href='#{Settings.m_host}/app/vips/exchanged?init_id=#{consume.id}'>#{point_gift.name}</a>])
  ...
  def send_coupon_care!
    ...
    random_receivers.take(limit_count).each do |vip_user|
      begin
        ...
        content.gsub!('{优惠券}', %Q[<a href="#{Settings.m_host}/#{vip_card.site_id}/coupons/my?openid=#{vip_user.user.wx_user.openid}">我的优惠券</a>])
