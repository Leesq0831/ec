...
include HasBarcode
  ...
  def self.export_excel(search)
    ...
    search.each do |s|
      sing_sheet << [s.vip_package.name, s.vip_package_item.name, s.sn_code, s.vip_user.try(:user_no), s.vip_user.try(:name), s.vip_user.try(:mobile), s.updated_at, s.shop_branch.try(:name)].flatten
    end
