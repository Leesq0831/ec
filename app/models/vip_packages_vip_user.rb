class VipPackagesVipUser < ActiveRecord::Base
  ...
  def self.export_excel(search)
    ...
    search.each do |s|
      sing_sheet << [s.vip_package.name, s.vip_package.price, s.payment_type_name, s.vip_user.try(:user_no), s.vip_user.try(:name), s.vip_user.try(:mobile), s.created_at, s.shop_branch.try(:name) || '商户总部', s.description].flatten
    end
