class WebsiteMenu < ActiveRecord::Base
  ...
  def docking_hotel_order_url
    "#{Settings.hotel_host}/wehotel-all/#{website.site_id}/getOrderList?openId="
  end
  ...
  def pic_or_icon(options = {})
    ...
    if pic_key.present? && website.website_setting.home_template_id == 117
      img = "<img src='#{pic_url}' style='width:50px;height:50px'> </img>"
      result = ["<i class='fa'> #{img} </i>"]
    end
