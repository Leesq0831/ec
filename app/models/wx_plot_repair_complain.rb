class WxPlotRepairComplain < ActiveRecord::Base
  ...
  private
    ...
    def igetui
      RestClient.post("#{Settings.app_host}/v1/igetuis/igetui_app_message", {role: 'site', role_id: wx_plot.try(:site_id), token: wx_plot.try(:site).account.try(:token), messageable_id: self.id, messageable_type: 'WxPlotRepairComplain', source: 'wx_plot', message: '您有一笔新订单，请尽快处理。'})
    rescue => e
      Rails.logger.info "#{e}" 
    end
