...
def create_xml request_options = {}, sign = ''
  if request_options[:sub_mch_id].blank?
    "<xml>
    ...
    "<xml>
        ...
        <notify_url>#{request_options[:notify_url]}</notify_url>
        <trade_type>#{request_options[:trade_type]}</trade_type>
        <sub_appid>#{request_options[:sub_appid]}</sub_appid>
        <sub_mch_id>#{request_options[:sub_mch_id]}</sub_mch_id>
        <sub_openid>#{request_options[:openid]}</sub_openid>
     </xml>"
   end
end
...
def set_sign_params request_options = {}
  if request_options[:sub_mch_id].blank?
    ...
    [
      ...
      "notify_url=#{request_options[:notify_url]}",
      "trade_type=#{request_options[:trade_type]}",
      "sub_appid=#{request_options[:sub_appid]}",
      "sub_mch_id=#{request_options[:sub_mch_id]}",
      "sub_openid=#{request_options[:sub_openid]}"
    ]
  end
end
